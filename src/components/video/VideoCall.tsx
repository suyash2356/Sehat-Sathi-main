...
export default function VideoCall() {
  ...
  // Map logical sides: left=patient, right=doctor
  ...
  const leftName = callData?.patientDetails?.name || 'Patient';
  const rightName = callData?.doctorName || 'Doctor';
  const rightSpecialization =
    callData?.doctorSpecialization || 'Doctor';
  ...
  useEffect(() => {
    try {
      ...
      // ignore assignment errors in SSR
    }
  }, [leftStream, rightStream]);

  // Attach remote audio for voice mode
  useEffect(() => {
    if (mode === 'voice' && remoteAudioRef.current && remoteStream) {
      ...
      <div className="flex flex-col items-center justify-center h-full w-full max-w-md mx-auto space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col items-center gap-4 w-full">
          ...
          <Button variant="outline" className="w-full" asChild>
            <Link
              href={
                isInitiator ? '/doctor/dashboard' : '/map'
              }
            >
              Cancel & Return
            </Link>
  ...
  return (
    <div className="w-full max-w-6xl h-full flex flex-col gap-6 md:p-4">
      ...
      {/* Hidden audio element for voice mode —  always present for reliability */}
      <audio
        ref={remoteAudioRef}
        autoPlay
        playsInline
      ...
      <div className="absolute bottom-24 left-1/2 -translate-x-1/2 flex gap-4 pointer-events-none z-30">
        {isConnecting && (
          <div className="bg-blue-600/80 backdrop-blur-md text-white px-4 py-1.5 rounded-full text-xs font-bold flex items-center gap-2 shadow-xl border border-blue-400/30">
            <Loader2 className="h-3 w-3 animate-spin" /> ENCRYPTING
            CONNECTION...
          </div>
        )}
        {isConnected && (
